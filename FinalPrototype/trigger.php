...
// 		$conn = new mysqli($server, $username, $password, $base);
		...
		$req = mysqli_query($db, "SELECT MAX(num_triggered) as max FROM triggered;") or die(mysqli_error);
		...
		// $stmt = $conn->prepare("INSERT INTO `soundstudy`.`triggered` (`num_triggered`,`display`)	 VALUES (?, ?)");
//                 if($stmt)
//                 {
...
//                 }
         ...
         $stmt_mamp = $conn_mamp->prepare("INSERT INTO `ubiquitous`.`triggered` (`num_triggered`,`display`)	 VALUES (?, ?)");
                if($stmt_mamp)
                {
